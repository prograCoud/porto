...
if ( ! function_exists( 'porto_ultimate_heading_spacer' ) ) {
	...
	$main_heading  = str_replace( '``', '"', $main_heading );
	...
	if ( $main_heading ) {
		...
		if ( 'yes' == $enable_highlight ) {
			if ( '' == $enable_svghighlight ) {
			...
			} elseif ( 'yes' == $enable_svghighlight ) {
				$svg_shape = '';
				if ( 'scribble' == $highlight_svg ) {
				...
				} elseif ( 'underline_zigzag' == $highlight_svg ) {
				...
				} elseif ( 'underline' == $highlight_svg ) {
					$svg_shape = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 100" preserveAspectRatio="none"><path d="M4,74.8h499.3"></path></svg>';
				}
				$main_heading = '<span class="heading-highlight-before">' . esc_html( $before_heading ) . '</span><span class="heading-highlight-svg">' . wp_kses_post( $main_heading ) . '<span class="svg-highlight ' . esc_attr( $highlight_svg ) . '">' . $svg_shape . '</span></span><span class="heading-highlight-after">' . esc_html( $after_heading ) . '</span>';
			}
		}
		$output .= '<div class="porto-u-main-heading' . $main_heading_class . '"' . $main_heading_hlight . '><' . esc_html( $heading_tag ) . ' ' . trim( $type_plugin ) . ' ' . $main_heading_attrs_escaped . '>' . porto_filter_output( $main_heading ) . '</' . esc_html( $heading_tag ) . '></div>';
	}

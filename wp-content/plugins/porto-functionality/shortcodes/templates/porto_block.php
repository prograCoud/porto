...
if ( $id || $name ) {
	...
	if ( $post_id ) {
		...
		if ( current_user_can( 'edit_pages' ) && ! is_customize_preview() && (
			...
			$before_html = '<div class="pb-edit-link" data-title="' . sprintf( esc_html__( 'Edit %1$s: %2$s', 'porto-functionality' ), esc_attr( $builder_type ), esc_attr( get_the_title( $post_id ) ) ) . '" data-link="' . esc_url( $edit_link ) . '"';
			if ( ! empty( $atts['tracking'] ) ) {
				$tracking_title = esc_html__( 'To Track', 'porto-functionality' );

/**
 * Post Type Builder - WooCommerce Short Description
 * 
 * @since 2.3.0
 */
...
( function ( wpI18n, wpBlocks, wpBlockEditor, wpComponents ) {
    ...
    const PortoTBWooDesc = function ( { attributes, setAttributes, name, clientId } ) {

        useEffect(
            () => {
                if ( ! attributes.el_class || -1 !== porto_tb_ids.indexOf( attributes.el_class ) ) { // new or just cloned
                    const new_cls = 'porto-tb-woo-desc-' + Math.ceil( Math.random() * 10000 );
        ...
        if ( attributes.alignment || attributes.font_settings ) {
            ...
            internalStyle += portoGenerateTypographyCSS( fontAtts, selectorCls + ' p' );
        }
    ...
    registerBlockType( 'porto-tb/porto-woo-desc', {
        ...
        icon: 'porto',
        category: 'porto-tb',
        keywords: [ 'type builder', 'mini', 'card', 'post', 'text', 'excerpt', 'woocommerce', 'product' ],
        description: __( 'Display the short description of a product.', 'porto-functionality' ),
        attributes: {
            content_type: {
                type: 'string',
            },

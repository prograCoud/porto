...
jQuery( document ).ready( function ( $ ) {
	...
	if ( typeof elementor != 'undefined' ) {
		...
		elementor.on( 'document:loaded', function () {
			...
			if ( $euiWrapper.length && ! $euiWrapper.hasClass( 'porto-top-bar-items' ) ) {
				...
				var $euiPreview = $euiWrapper.find('.eui-stack  .eui-box').eq( 2 );
	...
	} ).on( 'click', '#elementor-panel-saver-button-publish, .eui-button-group > .eui-button:not(.eui-tooltip), .vcv-ui-navbar-controls-group[data-vcv-guide-helper="save-control"], .vc_navbar-frontend .vc_btn-save, .vc_navbar-frontend .vc_btn-save-draft', function ( e ) {
	...
	$( '.porto-builder-cond-wrap' ).on( 'change', '.condition-type', function ( e, is_first ) {
		var val = $( this ).val();
		$cond = $( this ).closest( '.porto-builder-condition' );
		if ( 'single' == val ) {
